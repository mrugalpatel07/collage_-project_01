...
$query->bindParam(':urole',$urole,PDO::PARAM_STR);
 ...
 $query->execute();
   ...
   if ($LastInsertId>0) {
    echo '<script>alert("Facaulty has been created.")</script>';
...
<head>
    ...
    <title>Gate Pass Management System | Add Facaulty</title>
        ...
        <!--  page-wrapper -->
          <div id="page-wrapper">
            <div class="row">
                 <!-- page header -->
                <div class="col-lg-12">
                    <h1 class="page-header">Add Facaulty</h1>
                </div>
                <!--end page header -->
            </div>
            <div class="row">
...
<div class="form-group"> <label for="exampleInputEmail1">username (used for login)</label> 
    ...
    <input type="text" name="mobileno"  class="form-control" required='true' pattern="[0-9]{10}" title="10 numeric characters only" maxlength="10"> </div>

    <div class="form-group"> <label for="exampleInputEmail1">Password</label> 
    <input type="password" name="upassword" value="" class="form-control" required='true'> </div>
